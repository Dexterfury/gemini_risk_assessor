...
class _LoginScreenState extends State<LoginScreen> {
  ...
  // Keep all existing methods (handleClickedButton, handlePhoneSignIn, etc.) unchanged
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: ResponsiveLayoutHelper.responsiveBuilder(
        context: context,
        mobile: _buildMobileLayout(authProvider, isLoading),
        tablet: _buildTabletLayout(authProvider, isLoading),
        desktop: _buildDesktopLayout(authProvider, isLoading),
      ),
    );
  }

  Widget _buildMobileLayout(
      AuthenticationProvider authProvider, bool isLoading) {
    return Padding(
      padding: ResponsiveLayoutHelper.responsivePadding(
        context,
        mobile: const EdgeInsets.symmetric(horizontal: 20.0, vertical: 40.0),
      ),
      child: SingleChildScrollView(
        ...
        child: Column(
          children: [
            SizedBox(
                height: ResponsiveLayoutHelper.heightPercent(context, 0.05)),
            _buildLogo(),
            _buildTitle(),
            SizedBox(
                height: ResponsiveLayoutHelper.heightPercent(context, 0.03)),
            _buildSubtitle(),
            SizedBox(
                height: ResponsiveLayoutHelper.heightPercent(context, 0.03)),
            ...
            SizedBox(
                height: ResponsiveLayoutHelper.heightPercent(context, 0.04)),
            _buildOrContinueWith(),
            SizedBox(
                height: ResponsiveLayoutHelper.heightPercent(context, 0.04)),
            _buildSocialAuthButtons(isLoading),
          ],
        ),
      ),
    );
  ...
  Widget _buildTabletLayout(
    ...
    return Center(
      child: SizedBox(
        ...
        child: _buildMobileLayout(authProvider, isLoading),
      ),
    );
  }
  ...
  Widget _buildLogo() {
    return SizedBox(
      height: ResponsiveLayoutHelper.heightPercent(context, 0.15),
  ...
  Widget _buildSignInButton(AuthenticationProvider authProvider) {
    return SizedBox(
      height: 50,
      width: ResponsiveLayoutHelper.widthPercent(context, 1),
      child: MainAppButton(
  ...
  Widget _buildSocialAuthButtons(bool isLoading) {
    return isLoading
